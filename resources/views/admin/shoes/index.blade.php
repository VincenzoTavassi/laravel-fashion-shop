...
@section('content')
    <section class="container">
        ...
        <h1 class="my-5">{{isset($shoes) ? 'Scarpe' : 'Cestino'}}</h1>
        @if(isset($shoes))
        <a href="{{ route('admin.shoes.create') }}" class="btn btn-primary">Crea</a>
        <a href="{{ url('admin/shoes/trash') }}" class="btn btn-primary">Cestino</a>        
        @endif
        ...
        <table class="table my-3">
            <thead>
                ...
                <th scope="col">Anteprima</th>
                    ...
                    <th scope="col">
                        <a href="{{Request::url()}}?sort=deleted_at&order={{$sort=='deleted_at' && $order != 'desc' ? 'desc' : 'asc'}}">
                            Data Eliminazione
                            @if($sort == 'deleted_at')
                            <a href="{{Request::url()}}?sort=deleted_at&order={{$sort=='deleted_at' && $order != 'desc' ? 'desc' : 'asc'}}">
                                <i class="fa-solid fa-caret-down ms-2 d-inline-block @if ($order == 'desc') rotate-180 @endif">
            ...
            <tbody>
                ...
                @forelse($shoes as $shoe)
                    <tr>
                        ...
                        <td>€ {{ $shoe->price }}</td>
                        <td>
                            <a href="{{ route('admin.shoes.show', $shoe) }}" title="Mostra il prodotto"><i class="fa-solid fa-eye"></i></a>
                            <a href="{{ route('admin.shoes.edit', $shoe) }}" title="Modifica il prodotto"><i
                                    class="fa-solid fa-pen-to-square ms-3"></i></a>
                            <button class="fa-solid fa-trash btn-icon ms-3 text-danger" data-bs-toggle="modal" title="Elimina il prodotto"
